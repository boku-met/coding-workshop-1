...
authenticate at GitHub. The simplest way is using the GitHub command
line tool [`gh`](https://cli.github.com/). After installation, do `gh
auth login` whenever you want to install some repository with anything
similar to `pip install git+https://github.com/boku-met/...`. To verify
that you are good to go, try running the below:
...
Finally, prepare means to view and execute Jupyter Notebooks. All modern
integrated development environments (IDEs) can do that. However, you can
also use a conda environment with Jupyter installed and a browser. In
that case, run `jupyter lab` or `jupyter notebook <notebook>.ipynb` and
follow the instructions to access it in your browser.
